...
place_bracket_order_bp = Blueprint("place_bracket_order", __name__)

# expiries = ["1 Day", "3 Day", "7 Day", "30 Day"]

# ✅ Endpoints
@place_bracket_order_bp.route("/place-bracket-order/order-type", methods=["GET"])
@token_required
def get_order_type(user_id):
    return jsonify(fetch_values("SELECT order_type_id, order_type from order_type WHERE status=1"))

# @place_order_bp.route("/place-bracket-order/calls-puts", methods=["GET"])
# @token_required
# def get_calls_puts(user_id):
#     return jsonify(fetch_values("SELECT id,calls_puts FROM calls_puts where status=1"))

# @place_order_bp.route("/place-bracket-order/expiry", methods=["GET"])
# @token_required
...
# @place_order_bp.route("/place-bracket-order/strike-selection", methods=["GET"])
# @token_required
...
# @place_order_bp.route("/place-bracket-order/qty-type", methods=["GET"])
# @token_required
...
# @place_order_bp.route("/place-bracket-order/qty-percent", methods=["GET"])
# @token_required
...
# @place_order_bp.route("/place-bracket-order/qty-unit", methods=["GET"])
# @token_required
# def get_lots(user_id):
#     return jsonify(fetch_values("SELECT qty_unit_id, quantity_unit_name FROM quantity_unit where status=1"))


# @place_order_bp.route("/place-bracket-order/order-preview-lists", methods=["GET"])
# @token_required
...
# @place_order_bp.route("/place-bracket-order/placed-order", methods=["POST"])
# @token_required
